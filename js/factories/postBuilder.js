class postFactory {
    formatPost(element){ 
        ...
        let post = `
        <a><figure id="${element.id}" tabindex="0" class="ph-media focusable" aria-label="le media s'appelle ${element.title} et représente${element.alt}" data-name="${element.title}" ></figure>
        <div class="ph-work-elt-text">
            ...
            <div class='ph-work-like-div'>
                ...
                <i class="far fa-heart heart-btn focusable" aria-label='cliquez si vous avez aimé ${element.title}' role="button" data-value="${element.likes}" tabindex="0"></i>
            </div>    
        </div>
        `
    ...
    photoPost(element, photographer){
        ...
        let image = `<img src="resources/img/${photographer}/${element.image}" alt="${element.alt}" tabindex="-1">`
        place.innerHTML = image
    }

    videoPost(element, photographer){
        let place = document.getElementById(element.id);
        let video = `<video src="resources/img/${photographer}/${element.video}" alt="${element.alt}" tabindex="-1"></video>`
